...
$(document).ready(function() {
	//When new prodctsTable is set by Chrome extention, displays products and delete the productsTable
	$('#j-section-haouta-en-attente-post-product-scripts').bind("DOMSubtreeModified", function(){haoutaDisplayProduct_Admin(true, haoutaEnAttentePostProductsTable);
		$(this).html('');
	});
	//When new prodctsTable is set by Ajax get, displays products and delete the productsTable
	$('#j-section-haouta-en-attente-get-product-scripts').bind("DOMSubtreeModified", function(){haoutaDisplayProduct_Admin(false, haoutaEnAttenteGetProductsTable);
		$(this).html('');
	});
...
//Displays products in Haouta; Called whenever there is a new productsTable either from Ajax get or Chrome extension 
function haoutaDisplayProduct_Admin(postCall, productsTable) {
	/** Sends new product data **/
	if(postCall) {
		postHaoutaEnAttent();
	}
	/*******DOM blocks and elements creation*******/
	for(var k = 0; k < productsTable.length; k++){
		variantsData = productsTable[k];
		...
		/*****Product variants*****/
		if(variantsData.variants_html.length){
